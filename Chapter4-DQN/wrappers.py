...
cv2.ocl.setUseOpenCL(False) # 此处禁用opencl这个gpu加速技术


def build_env(env_id, seed, reward_scale=1.0):
...
class NoopResetEnv(gym.Wrapper):
    ...
    def __init__(self, env, noop_max=30):
        ...
        No-op is assumed o be action 0.
        """
...
class EpisodicLifeEnv(gym.Wrapper):
    ...
    def __init__(self, env):
        """Make end-of-lif == end-of-episode, but only reset on true game over.
    ...
    def reset(self,**kwargs):
        ...
        This way all states are reachable when the life is episodic,
...
class MaxAndSkipEnv(gym.Wrapper):
    ...
    def step(self, action):
        ...
        for i in range(self._skip):
            ...
            if done:
                break
        # Note that the observation on the done=True frame doesn't doesn't matter.
...
class WarpFrame(gym.ObservationWrapper):
    ...
    def __init__(self, env, width=84, height=84, grayscale=True):
        """Warp frames to 84x84 as done in Nature paper and later work."""
        super(WarpFrame, self).__init__(env)
        self.width = width
        self.height = height
        self.grayscale = grayscale
        shape = (width, height, 1 if grayscale else 3)
...
class FrameStack(gym.Wrapper):
    ...
    def __init__(self, env, k):
        """Stack k last frames.
        Return lazy array, which is much more memory efficient.
        See also 'LazyFrames'
        """
...
class LazyFrames(object):
    ...
    def __init__(self, frames):
        """This object ensures that common frames between observations are
        only stored once. It exists only to optimize memory usage which can be
...
class RewardScaler(gym.Wrapper):
    ...
    def __init__(self, env, scale=0.01):
        ...
        """
        super(RewardScaler).__init__(env)
        self.scale = scale
...
class TimeLimit(gym.Wrapper):
    ...
    def __init__(self, env, max_episode_steps=None):
        """Limit the max number of steps for each episode.
        This is a default wrapper for atari games.
        """
